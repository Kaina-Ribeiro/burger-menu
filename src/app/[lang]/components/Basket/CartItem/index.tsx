...
import { priceInternationalization } from '@/utils/priceInternationalization';
    ...
    <div className="p-4 leading-5 bg-white">
      <div className="flex justify-between text-black-100">
        ...
        <span className="font-medium">
          {priceInternationalization({ price, appInfo })}
        </span>
      </div>
      ...
      {modifiers?.map((modifier, i) => (
        <span className="font-normal text-base text-gray-300" key={i}>
          {modifier}{' '}
          {`(+${priceInternationalization({ price: quantity * price, appInfo })})`}
        </span>
      ))}
      ...
      <div className="flex items-center gap-2 mt-2">
        ...
        <button onClick={sum} className="bg-primary w-5 h-5 p-1 rounded-full">
          <Image src={PlusIcon} alt="plus-icon" width={12} height={12} />
        </button>
      </div>
    </div>
  );
