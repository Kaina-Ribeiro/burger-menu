...
import Icon from '@/assets/images/icons/header-hamburger.svg';
...
export default function HeaderMobile({
  ...
  const currentPath = { ...navigation.find((p) => pathName.includes(p.path)) };
  ...
  return (
    <nav className="relative flex items-center justify-center w-full py-[18px]">
      ...
      <button className="absolute right-0 hover:opacity-80">
        <Image src={Icon} width={16} height={16} alt="hamburger-icon" />
      </button>
    </nav>
  );
}
